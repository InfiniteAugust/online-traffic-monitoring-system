...
class vehicle_detector(threading.Thread):
    """接收视频流，并保存处理结果"""
    def __init__(self):
    ...
    def detecting(self, custom_objects=None, frame_detection_interval=1, minimum_percentage_probability=50, display_percentage_probability=True, display_object_name = True):
        '''检测图片，并保存边框'''
        if (not self.video):
            raise ValueError(
                "No video source")
    ...
    def run(self):
        ...
        self.detecting(custom_objects=custom_objects)